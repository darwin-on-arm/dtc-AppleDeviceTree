			 ...
			 struct version_info *vi)
{
	...
	if ((vi->flags & FTF_NAMEPROPS) && !seen_name_prop) {
		...
		printf("This path not handled yet\n");
		abort();
...
void dt_to_blob(FILE *f, struct boot_info *bi, int version)
{
    ...
    int deviceTreeLength;
    void* deviceTreeP = NULL;

    DT__FlattenDeviceTree(NULL, &deviceTreeLength);
    assert((deviceTreeP = malloc(deviceTreeLength)));
    DT__FlattenDeviceTree((void **)&deviceTreeP, &deviceTreeLength);

	if (fwrite(deviceTreeP, deviceTreeLength, 1, f) != 1) {
		if (ferror(f))
